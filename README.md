...
1. **みんなが使いやすい工夫**: 画面上の説明やキーボードだけでも操作できる仕組みを取り入れ、誰でも簡単に使えるようにしています。特に、プログラミング言語で絞り込む：やスター数で絞り込むボタンを追加したり、GitHubの制限により最初の1000件のみ表示可能にしている所と表示出来ない所は、表示しないように工夫しています。1000件を超えた場合には、検索結果を絞り込むためのヒント：を表示するようにしています。
